...
def cargar_imagen(nombre_archivo):
    ...
    Returns:
        pygame.Surface: La superficie de la imagen cargada o una de error.
    """
    try:
...
def cargar_sonido(nombre):
    ...
    Returns:
        pygame.mixer.Sound or None: El objeto de sonido o None si falla.
    """
    try:
...
def generar_animacion_moneda(tamano, cara_img, sello_img):
    """
    Genera programáticamente los frames de una animación de giro de 360 grados.
    Simula una rotación 3D escalando el ancho de la imagen con una función coseno.

    Args:
        tamano (int): El tamaño (ancho y alto) de la moneda.
        cara_img (pygame.Surface): La imagen del lado "cara".
        sello_img (pygame.Surface): La imagen del lado "sello".

    Returns:
        list: Una lista de superficies de Pygame, cada una es un frame de la animación.
    """
    imagenes = []
...
class Particula:
    """Clase para crear un efecto de partículas (chispas) al ganar."""
    ...
    def dibujar(self, superficie):
        """Dibuja la partícula en la pantalla si aún está viva."""
        if self.vida > 0:
            pygame.draw.circle(superficie, self.color, (int(self.x), int(self.y)), int(self.radio))

class Boton:
    """Clase para crear botones interactivos con texto y colores."""
    ...
    def dibujar(self, superficie):
        """Dibuja el botón, cambiando de color según el estado."""
...
def juego_coinflip():
    """Función principal que contiene el bucle y la lógica del juego Cara o Sello."""
    pygame.init()
    pygame.mixer.init()
    ...
    fondo_img = cargar_imagen(os.path.join("Fondos", "fondocoinflip.jpg")).convert()
    cara_img_original = cargar_imagen(os.path.join("Moneda", "cara.png"))
    sello_img_original = cargar_imagen(os.path.join("Moneda", "sello.png"))
