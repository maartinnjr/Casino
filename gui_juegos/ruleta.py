...
# --- Variables Globales de Pygame (se inicializan en juego_ruleta) ---
fuente = None
fuente_titulo = None
...
class Boton:
    ...
    def __init__(self, x, y, texto, color_normal, color_hover, fuente_boton):
        ...
        Args:
            ...
            color_normal (tuple): Color RGB del botón en estado normal.
            color_hover (tuple): Color RGB del botón cuando el cursor está encima.
            fuente_boton (pygame.font.Font): Fuente del texto.
        """
        self.texto = texto
    ...
    def dibujar(self, superficie):
        """
        Dibuja el botón, cambiando de color si está seleccionado o si el cursor está encima.
    ...
    def verificar_clic(self, evento):
        ...
        Returns:
            bool: True si se hizo clic, False en caso contrario.
        """
...
def dibujar_ruleta(ventana_surf, angulo, centro_ruleta, radio, sectores, angulo_sector):
    """
    Dibuja la ruleta sector por sector, rotada según el ángulo actual.

    Args:
        ventana_surf (pygame.Surface): Superficie donde dibujar la ruleta.
        angulo (float): Ángulo de rotación actual en grados.
        centro_ruleta (tuple): Coordenadas (x, y) del centro.
        radio (int): Radio de la ruleta.
        sectores (int): Número de divisiones de la ruleta.
        angulo_sector (float): Ancho angular de cada sector en grados.
    """
...
def dibujar_flecha_fija_imagen(ventana_surf, flecha_img_val, centro_ruleta, radio):
    """
    Dibuja la imagen de la flecha fija que indica el sector ganador.

    Args:
        ventana_surf (pygame.Surface): Superficie donde dibujar.
        flecha_img_val (pygame.Surface): La imagen de la flecha.
        centro_ruleta (tuple): Coordenadas del centro de la ruleta.
        radio (int): Radio de la ruleta.
    """
...
def juego_ruleta():
    """
    Función principal que contiene el bucle y la lógica del juego de la ruleta.
    """
    pygame.init()
    pygame.mixer.init()
    ...
    fondo_img = pygame.image.load(resource_path(os.path.join("img", "Fondos", "fondoruleta.jpg"))).convert()
    flecha_img = pygame.image.load(resource_path(os.path.join("img", "Ruleta", "flecha.png"))).convert_alpha()
    ...
    while corriendo:
        ...
        for evento in pygame.event.get():
            if evento.type == pygame.QUIT:
                if estado == "esperando": crear_resultado("empate")
                else: crear_resultado("perdiste")
                corriendo = False
