...
class SlotMachine:
    """
    Representa el Modelo (la lógica) de la máquina tragamonedas.
    Maneja los símbolos, multiplicadores, el giro y el cálculo de ganancias.
    """
    def __init__(self):
        """Inicializa la máquina con sus símbolos y multiplicadores de premio."""
        self.multipliers = {
            "🍇": 3, "🍒": 4, "🍋": 5,
            "🍊": 6, "🍉": 8, "🍓": 10
        }
    ...
    def spin(self):
        """Simula un giro, generando un nuevo resultado aleatorio de tres símbolos."""
        self.result = [random.choice(self.symbols) for _ in range(3)]

    def calculate_winnings(self, bet, chosen_fruit):
        """
        Calcula las ganancias basadas en el resultado del giro y la fruta elegida.

        Args:
            bet (int): La cantidad apostada.
            chosen_fruit (str): La fruta que el jugador seleccionó como su "fruta de la suerte".

        Returns:
            tuple: (int, str) conteniendo la ganancia neta y un mensaje descriptivo.
        """
...
class SlotMachineView:
    """
    Representa la Vista (la interfaz gráfica) de la máquina tragamonedas.
    Construida con Tkinter, se encarga de mostrar los elementos visuales.
    """
    def __init__(self, root, controller, bet_amount):
        ...
        Args:
            root (tk.Tk): La ventana raíz de Tkinter para este juego.
            controller (SlotMachineController): El controlador que manejará la lógica.
            bet_amount (int): El monto de la apuesta actual.
        """
    ...
    def setup_background(self):
        """Carga y establece la imagen de fondo para la ventana del juego."""
        try:
            image_path = resource_path(os.path.join("img", "Fondos", "fondotragamonedas.jpg"))
    ...
    def setup_game_ui(self):
        """Crea y posiciona los widgets principales del juego (rodillos, botones, etc.)."""
    ...
    def setup_prizes_ui(self):
        """Crea y muestra la tabla de premios en el panel lateral."""
    ...
    def highlight_selected_fruit(self, selected_fruit):
        """Resalta visualmente el botón de la fruta seleccionada por el jugador."""
    ...
    def animate_reels(self, final_result, sound_stop):
        """Ejecuta una animación visual de los rodillos girando antes de mostrar el resultado."""
    ...
    def show_final_screen(self, message, net_winnings):
        """
        Prepara la pantalla de resultado final después de un breve retraso.

        Args:
            message (str): El mensaje principal del resultado (ej: "¡Trío de 🍓!").
            net_winnings (int): La ganancia o pérdida neta.
        """
    ...
    def create_result_dialog(self, message, net_winnings):
        """Crea una ventana modal superpuesta para mostrar el resultado final del juego."""
...
class SlotMachineController:
    """
    Representa el Controlador (la conexión entre Modelo y Vista).
    Maneja las acciones del usuario (clics, teclas) y actualiza la vista y el modelo.
    """
    ...
    def _load_sound(self, filename):
        """Carga un archivo de sonido de forma segura."""
        try:
    ...
    def get_symbols(self):
        """Devuelve la lista de símbolos disponibles desde el modelo."""
        return self.model.symbols
    
    def get_multipliers(self):
        """Devuelve el diccionario de multiplicadores desde el modelo."""
        return self.model.multipliers

    def select_fruit(self, fruit):
        """
        Maneja la selección de una fruta por parte del jugador.

        Args:
            fruit (str): El símbolo de la fruta seleccionada.
        """
    ...
    def play(self):
        """
        Ejecuta la lógica principal del juego: girar, animar y mostrar resultados.
        """
        if self.game_played:
            return
        ...
        if self.model.result == ["🍓", "🍓", "🍓"]:
            if self.sound_strawberry_win: self.sound_strawberry_win.play()
        elif net_winnings > 0:
            if self.sound_win_fruit: self.sound_win_fruit.play()
        elif net_winnings == 0:
            if self.sound_tie: self.sound_tie.play()
        else:
            if self.sound_lose: self.sound_lose.play()

        self.view.show_final_screen(message, self.net_winnings)

    def salir(self):
        """
        Cierra la ventana del juego y escribe el resultado en 'resultado.txt'.
        Si el juego no se jugó, escribe -1. De lo contrario, escribe la ganancia bruta.
        """
        if not self.game_played:
